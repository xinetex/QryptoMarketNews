...
export default function FlexInnovationWidget() {
    ...
    return (
        <>
            ...
            <motion.div
                layoutId="flex-innovation-card"
                onClick={() => setIsOpen(true)}
                className="cursor-pointer group relative bg-gradient-to-br from-zinc-900 to-black border border-white/10 rounded-2xl p-5 overflow-hidden h-44 flex flex-col justify-between z-0"
                whileHover={{ scale: 1.02 }}
            >
                {/* Content opacity handles the 'disappear' effect during morph */}
                <motion.div className="flex flex-col h-full justify-between" animate={{ opacity: isOpen ? 0 : 1 }}>
            ...
            <AnimatePresence>
                {isOpen && (
                    <Portal>
                        ...
                        {/* Panel Container (Zoom/Morph Target) */}
                        <motion.div
                            layoutId="flex-innovation-card"
                            className="fixed top-0 right-0 h-full w-full md:max-w-md bg-zinc-950 border-l border-white/10 shadow-2xl z-[101] overflow-hidden flex flex-col"
                        >
